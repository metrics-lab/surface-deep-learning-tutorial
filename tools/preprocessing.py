...
def main(config):
    ...
    ids = pd.read_csv(os.path.join(label_path, '{}/{}/half/{}.csv'.format(modality,task,split)))['ids'].to_numpy().reshape(-1)
    labels = pd.read_csv(os.path.join(label_path,'{}/{}/half/{}.csv'.format(modality,task,split)))['labels'].to_numpy().reshape(-1)
    num_subjects = ids.shape[0]

    means = np.load(os.path.join(config['data']['label_path'],'{}/{}/half/{}/means.npy'.format(modality,task,configuration)))
    stds = np.load(os.path.join(config['data']['label_path'],'{}/{}/half/{}/stds.npy'.format(modality,task,configuration)))
    ...
    for i,id in enumerate(ids):
        print(id)
        filename = os.path.join(path_to_data,'regression_{}_space_features'.format(configuration),'{}_{}_L.shape.gii'.format(id.split('_')[0],id.split('_')[1]))
        data.append(np.array(nb.load(filename).agg_data())[:num_channels,:])
        filename = os.path.join(path_to_data,'regression_{}_space_features'.format(configuration),'{}_{}_R.shape.gii'.format(id.split('_')[0],id.split('_')[1]))
        data.append(np.array(nb.load(filename).agg_data())[:num_channels,:])
    ...
    indices_mesh_triangles = pd.read_csv('../patch_extraction/msm/triangle_indices_ico_{}_sub_ico_{}.csv'.format(ico,sub_ico))

    #shape of the data is nbr_subjects * 20 trianglas * nbr_vertices_per_triangle * channels
    new_data = np.zeros((num_subjects*2, num_channels, num_patches, num_vertices))

    for i,id in enumerate(ids):
        print(id)
        for j in range(num_patches):
            indices_to_extract = indices_mesh_triangles[str(j)].to_numpy()
            new_data[i,:,j,:] = normalised_data[2*i][:,indices_to_extract]
            new_data[i+num_subjects,:,j,:] = normalised_data[2*i+1][:,indices_to_extract]
