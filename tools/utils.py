    ...
    if str(dataloader) in ['metrics','numpy']:
        if dataset == 'dHCP':
            ...
            if modality == 'cortical_metrics':
                ...
                if registration == 'msmsulc':
                    data_path = os.path.join(config['data']['path_to_data'],dataset,'rsns/all_msmsulc/')
                else:
            ...
            if modality == 'cortical_metrics':
                ...
                data_path = os.path.join(config['data']['path_to_data'],dataset,'merged_metrics/ico6_{}_{}'.format(sampling,configuration), '{}_{}'.format(modality, registration))
            elif '3T_tfMRI' in modality:
                if registration == 'msmsulc':
                    data_path = os.path.join(config['data']['path_to_data'],dataset,'HCP_1200/' )
                elif registration == 'msmall':
                    data_path = os.path.join(config['data']['path_to_data'],dataset,'HCP_1200/' )
...
def get_dataloaders(config, 
    ...
    if str(dataloader)=='metrics':
        if str(modality) == 'cortical_metrics' or str(modality) == 'memory_task' or str(modality) == 'rsns' or '3T_tfMRI' in str(modality):
            train_loader, val_loader, test_loader = loader_metrics(data_path,sampler,config)
    elif str(dataloader)=='functional':
...
def logging_sit(config, pretraining=False):
    ...
    if config['data']['subset']:
        folder_to_save_model = folder_to_save_model + '-subset'
...
def save_reconstruction_diffusion(reconstructed_example,
                                input_example, 
                                num_patches,
                                num_vertices,
                                ico_grid,
    ...
    try:
        os.makedirs(os.path.join(folder_to_save_model, 'reconstruction'),exist_ok=False)
        print('Creating folder: {}'.format(folder_to_save_model))
    except OSError:
        pass
    ...
    new_inputs = np.transpose(input_example.cpu().numpy(),(0,2,1,3))

    for i in range(num_patches):
        indices_to_extract = indices[str(i)].values
        original_sphere[indices_to_extract,:] = new_inputs[0,i,:,:].transpose()

    save_gifti(original_sphere, os.path.join(folder_to_save_model,'reconstruction','original_sphere.shape.gii'))

    B, num_masked_patch, V = reconstructed_example.shape
    rearrange_layer_masked = Rearrange('b m (v c) -> b c m v', b=B, m=num_masked_patch, c=num_channels, v=num_vertices)

    batch = rearrange_layer_masked(reconstructed_example).cpu().numpy()

    reconstructed_sphere = np.zeros((40962,num_channels),dtype=np.float32)

    for i in range(num_patches):
        indices_to_extract = indices[str(i)].values
        reconstructed_sphere[indices_to_extract,:] = batch[0,:,i,:].transpose()

    save_gifti(reconstructed_sphere, os.path.join(folder_to_save_model,'reconstruction', 'reconstructed_sphere.shape.gii'))
