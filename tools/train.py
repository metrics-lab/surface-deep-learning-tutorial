...
sys.path.append('/home/sd20/workspace/github/metrics-lab/surface-deep-learning-tutorial/')
...
from tools.utils import logging_sit, logging_ms_sit, plot_regression_results_UKB, plot_regression_results_dHCP, plot_regression_results_HCP
...
def train(config):
    ...
    for epoch in range(epochs):
        ...
        if classification_task:  
            ...
            stds = np.std(np.abs(np.concatenate(targets_) - np.concatenate(preds_)))
        ...
        if (epoch+1)%val_epoch==0:
            ...
            if classification_task:  
                ...
                std_val_epoch = np.std(np.abs(np.concatenate(targets_)- np.concatenate(preds_)))
                ...
                print('| Validation | Epoch - {} | Loss - {:.4f} | MAE - {:.4f} |  STD - {:.4f} | Corr - {:.4f} | R2 - {:.4f} |'.format(epoch+1, loss_val_epoch, mae_val_epoch, std_val_epoch, correlation_val, r2_val_epoch))
