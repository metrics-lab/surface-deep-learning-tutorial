...
class dataset_cortical_surfaces(Dataset):
    def __init__(self, 
        ...
        if config['data']['registration'] == 'msmall':
            self.registration = 'MSMAll'
        elif config['data']['registration'] == 'msmsulc':
            self.registration = 'MSMSulc'
        ...
        if config['MODEL'] == 'sit' or config['MODEL']=='ms-sit':
            ...
            self.patching=False 
            self.channels = config['mlp']['channels']
            self.num_channels = len(self.channels)
        else:
        ...
        if config['data']['dataset']=='UKB':
            ...
            if config['data']['subset']:                                                                   
                self.data_info = pd.read_csv('{}/labels/{}/{}/{}_{}/{}/{}_sub.csv'.format(config['data']['path_to_workdir'],
                                                                            self.dataset,
                                                                            self.modality,
                                                                            config['data']['task'],
                ...
                self.data_info = pd.read_csv('{}/labels/{}/{}/{}_{}/{}/{}.csv'.format(config['data']['path_to_workdir'],
                                                                            self.dataset,
                                                                            self.modality,
                                                                            config['data']['task'],
                                                                            config['data']['registration'],
        ...
        elif '3T_tfMRI' in self.modality and config['data']['dataset']=='HCP':

            modality = '_'.join(self.modality.split('_')[:2])
            fmri_task = self.modality.split('_')[2]

            self.data_info = pd.read_csv('{}/labels/{}/{}/{}/{}/{}/{}.csv'.format(config['data']['path_to_workdir'],
                                                                            self.dataset,modality,fmri_task,task,
                                                                            self.hemi,
                                                                            split))
    ...
    def get_half_hemi(self,idx):
        ...
        if '3T_tfMRI' in self.modality:
            id = self.filenames.iloc[idx].split('_')[0]
            fmri_task = self.filenames.iloc[idx].split('_')[2]
            path = os.path.join(self.filedir,id,'MNINonLinear/Results/tfMRI_{}/tfMRI_{}_hp200_s2_level2_{}.feat'.format(fmri_task,fmri_task,self.registration),
