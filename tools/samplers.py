...
def sampler_HCP_scan_age(labels):
    total = len(labels)
    frac_0 = total / np.sum(labels[:]>=33) ## before I was dividing that by 2

    weights = np.ones(len(labels)) * frac_0

    frac_1  = (total/ np.sum(labels[:]<26))  #before i was diving by 2
    frac_2 = total/ (np.sum(labels[:]<33) - np.sum(labels[:]<=26))
    weights[np.where(labels[:]<26)] = frac_1
    weights[np.where(np.logical_and(labels[:]<33 , labels[:]>=26))] = frac_2
    weights = torch.DoubleTensor(weights)                                       
    sampler = torch.utils.data.sampler.WeightedRandomSampler(weights, len(weights))
    print('Above 75 {}'.format(round(frac_0,2)), 'below 26 {}'.format(round(frac_1,2)), 'between 26 and 33 {}'.format(round(frac_2,2)))
